...
| Timezone, NTP servers, nightly neighbour scan | ✅ Working (unit-tested; not yet seen applied on hardware). The site timezone and NTP servers from every full push go to UCI `system`, with the board's originals stamped alongside (`openuf_*_orig`). The controller's nightly `syswrapper.sh 11k-scan` cron job is installed in a marked block of root's crontab. That job scans every radio on the next heartbeat, and the fresh neighbours ride out on that inform. Only commands openUF provides are ever put in cron (see USAGE § 6) |
