...
| Environment / rogue-AP scanning (`scan_radio_table`) | ✅ Working — confirmed rendering live in the Environment tab. Read from the kernel's passive BSS cache, so it lists neighbours on the radio's **own** channel and never dwells off-channel; the full sweep is the controller's RF scan |
